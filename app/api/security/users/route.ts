...
export async function GET(request: Request) {
  ...
  try {
    if (ids.length > 0) {
      const results = await Promise.all(
        ids.map(async (userId) => {
          try {
            const response = await apiClient.security.getUserByUserId({ userId });
    ...
    const response = await apiClient.security.getUserByPaging({
      page,
      pageSize,
      searchTerm: search
    });
    ...
    if (error instanceof ApiError) {
      ...
      const message = body?.message ?? body?.errors?.general?.[0] ?? "Unable to load users";
